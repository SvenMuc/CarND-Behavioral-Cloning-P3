...
def visualize_generator_data_set(samples, title='', show=True):
    ...
    network = NvidiaCNN(input_depth=3, input_height=64, input_width=64, regression=True, nb_classes=1,
                        roi=roi, steering_angle_correction=6.25, skip_rate_zero_angles=0.7, angle_threshold=0.1)

    network.path_to_image_data = './data'
    network.verbose = 1

    generator = network.generator(samples)
    nb_generator_samples = network.get_number_genertor_samples(samples)

    print("Preparing dataset by generator...", end='', flush=True)

    for X_train, y_train in generator:
        angles_generator = np.append(angles_generator, y_train * 25.)

        if len(angles_generator) >= nb_generator_samples:
            print('Done')
            break
...
def compare_model_prediction(samples, model, dataset_path='', show=True):
    ...
    for sample in samples:
        ...
        angles_left.append(float(sample[3]) * 25. + 6.25)
        angles_right.append(float(sample[3]) * 25. - 6.25)
        ...
        center_image = BaseNetwork.preprocess_image(center_image, 64, 64, roi)
        left_image = BaseNetwork.preprocess_image(left_image, 64, 64, roi)
        right_image = BaseNetwork.preprocess_image(right_image, 64, 64, roi)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        ...
        visualize_generator_data_set(samples, title='Generator Steering Angle Histogram')
