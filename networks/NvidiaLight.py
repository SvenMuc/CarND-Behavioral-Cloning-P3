...
class NvidiaLight(BaseNetwork):
    """ Lightweight NVIDIA CNN (smaller layer) network which can be used either for a classification or 
    regression problem."""
    ...
    def __init__(self, input_width, input_height, input_depth, nb_classes, regression=False,
        ...
        super(NvidiaLight, self).__init__('NvidiaLight', input_width, input_height, input_depth, nb_classes, regression,
                                         roi, steering_angle_correction, angle_threshold, weights_path)

        print('NVIDIA Light CNN Configuration:')
    ...
    def setup_network_architecture(self):
        """ Constructs the lightweight NVIDIA CNN network architecture.
        ...
        self.model.add(Convolution2D(3, 5, 5,  border_mode='same', activation='elu'))
        self.model.add(MaxPooling2D(pool_size=(2, 2), strides=(2, 2)))
        self.model.add(Convolution2D(24, 5, 5, border_mode='same', activation='elu'))
        self.model.add(MaxPooling2D(pool_size=(2, 2), strides=(2, 2)))
        self.model.add(Convolution2D(36, 5, 5, border_mode='same', activation='elu'))
        self.model.add(MaxPooling2D(pool_size=(2, 2), strides=(2, 2)))
        self.model.add(Convolution2D(48, 3, 3, border_mode='same', activation='elu'))
        self.model.add(MaxPooling2D(pool_size=(2, 2)))
        #self.model.add(Convolution2D(64, 3, 3, border_mode='same', activation='elu'))
        #self.model.add(MaxPooling2D(pool_size=(2, 2)))
